<h1 align="center" style="font-weight: bold">
    DCIT50 Lab 3
</h1>

<p align="center">
    <a href="https://github.com/whinee/DCIT50-Lab-3/issues">
        <img src="https://img.shields.io/github/issues/whinee/DCIT50-Lab-3.svg?style=for-the-badge&logo=data:image/png;base64,">
    </a>
    <a href="https://github.com/whinee/DCIT50-Lab-3/network/members">
        <img src="https://img.shields.io/github/forks/whinee/DCIT50-Lab-3.svg?style=for-the-badge&logo=data:image/png;base64,">
    </a>
    <a href="https://github.com/whinee/DCIT50-Lab-3/stargazers">
        <img src="https://img.shields.io/github/stars/whinee/DCIT50-Lab-3.svg?style=for-the-badge&logo=data:image/png;base64,">
    </a>
    <a href="https://github.com/whinee/DCIT50-Lab-3/graphs/contributors">
        <img src="https://img.shields.io/github/contributors/whinee/DCIT50-Lab-3.svg?style=for-the-badge&logo=data:image/png;base64,">
    </a>
    
</p>

<p align="center">
    <a href="LICENSE.md">
        <img src="https://img.shields.io/badge/LICENSE-A31F34?style=for-the-badge&logoWidth=25&logo=data:image/png;base64,">
    </a>
</p>

Hello! This project is about simulating a vending machine.

## Getting Started

To get started, follow [this instruction](getting-started.md).

## Run

There are two options, singular command or series of commands for different OSes.

### Singular Command

```sh
just run
```

### Windows

```ps1
javac src/main/java/xyz/whinyaan/dcit50/lab3/*.java -Xdiags:verbose
java -cp src/main/java xyz.whinyaan.dcit50.lab3.App
```

### MacOS

```sh
set -euo pipefail
javac src/main/java/xyz/whinyaan/dcit50/lab3/*.java -Xdiags:verbose
java -cp src/main/java xyz.whinyaan.dcit50.lab3.App
```

### Linux

```sh
set -euo pipefail
javac src/main/java/xyz/whinyaan/dcit50/lab3/*.java -Xdiags:verbose
java -Dawt.useSystemAAFontSettings=lcd_hrgb -Dswing.aatext=true -cp src/main/java xyz.whinyaan.dcit50.lab3.App # set antialiasing to true for smooth font rendering
```

<!-- License section. Leave unchanged except when changing the style altogether. -->
<h2 id="license"><b><a href="#license">License</a></b></h2>

### <a target="_blank" href="https://choosealicense.com/licenses/mit/">MIT License</a>

Copyright 2024 whinee

Check the [LICENSE](LICENSE.md) for more details.

<h2 id="attribution"><b><a href="#attribution">Attribution</a></b></h2>

<h2 id="attribution-mit-logo"><b><a href="#attribution-mit-logo">MIT Logo</a></b></h2>

<a target="_blank" href="https://commons.wikimedia.org/wiki/File:MIT_logo.svg">Massachusetts Institute of Technology</a> (vectorized by <a target="_blank" href="https://en.wikipedia.org/wiki/User:Mysid">Mysid</a>, modified by [whinee](https://github.com/whinee)), Public domain, via Wikimedia Commons

<h2 id="attribution-icons"><b><i><a href="#attribution-icons">Icons</a></i></b></h2>

<a target="_blank" href="https://icons8.com/icon/102502/exclamation-mark">Exclamation Mark</a>, <a target="_blank" href="https://icons8.com/icon/33294/code-fork">Code Fork</a>, <a target="_blank" href="https://icons8.com/icon/85185/star">Star</a>, <a target="_blank" href="https://icons8.com/icon/34095/group">Group</a>, and <a target="_blank" href="https://icons8.com/icon/87276/code">Code</a> icon by <a target="_blank" href="https://icons8.com">Icons8</a>
